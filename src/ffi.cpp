...
bool ffi::try_marshal(lisp_value val, void **out_ptr) {
    if (val.is_type(SYSTEM_POINTER_TYPE)) {
        *out_ptr = val.as_object()->ptr();
        return true;
    }
    if (val.is_fixnum()) {
...
void *ffi::marshal_and_call(void *f, lisp_value *args, uint32_t nargs)
{
